...
如果需要修改代码：
1. 修改代码之前，确保当前代码能够通过编译和所有测试用例
2. 使用 Edit 或 Write 工具修改代码
3. **绝对不要执行任何 git 命令**
4. 确保修改的代码符合项目规范和质量标准
5. 代码修改完成后，确保能够通过编译和所有测试用例`,
...
[RESULT]
code_changed: true | false # 是否修改了代码
...
export function parseResult(response: string): CommentResult | null {
  ...
  // Parse branch_name (optional)
  let branch_name: string | undefined;
  const branchNameMatch = block.match(/branch_name:\s*(.+)/i);
  if (branchNameMatch) {
    branch_name = branchNameMatch[1].trim().replace(/^"|"$/g, '');
  }

  return { code_changed, summary, changed_files, commit_message, branch_name };
}
