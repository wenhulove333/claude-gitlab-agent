...
## 输出要求
回答后，**必须**输出以下结构化信息（使用此精确格式，以便程序解析）：
...
// 从响应中解析 [RESULT]...[/RESULT] 块
function parseCommentResult(response: string): CommentResult | null;
```
...
**结构化解析**：
```typescript
interface StructuredIssueAnalysis {
  category: 'new_feature' | 'improvement' | 'bug_fix' | 'not_related' | 'unknown';
  summary: string;
}

// 从响应中解析 [ANALYSIS]...[/ANALYSIS] 块
function parseStructuredIssueAnalysis(response: string): StructuredIssueAnalysis | null;
```
